...
float GoodCitizen::pyramid_vertices[] = {
		// FRONT
		-2.0f, -2.0f, 2.0f, 2.0f, -2.0f, 2.0f, 0.0f, 2.0f, 0.0f,
		// BACK
		2.0f, -2.0f, -2.0f, -2.0f, -2.0f, -2.0f, 0.0f, 2.0f, 0.0f,
		// LEFT
		-2.0f, -2.0f, -2.0f, -2.0f, -2.0f, 2.0f, 0.0f, 2.0f, 0.0f,
...
float GoodCitizen::pyramid_normals[] = {
		// FRONT
		0.0f, 0.6667f, 0.5f, 0.0f, 0.6667f, 0.5f, 0.0f, 0.6667f, 0.5f, 0.0f,
		0.6667f, 0.5f,
		// BACK
		0.0f, 0.6667f, -0.5f, 0.0f, 0.6667f, -0.5f, 0.0f, 0.6667f, -0.5f, 0.0f,
		0.6667f, -0.5f,
		// LEFT
		-0.5f, 0.6667f, 0.0f, -0.5f, 0.6667f, 0.0f, -0.5f, 0.6667f, 0.0f, -0.5f,
		0.6667f, 0.0f,
		// RIGHT
		0.5f, 0.6667f, 0.0f, 0.5f, 0.6667f, 0.0f, 0.5f, 0.6667f, 0.0f, 0.5f,
		0.6667f, 0.0f,
...
GoodCitizen::GoodCitizen(VIEW_DISPLAY display) {
	...
	for (phistep = 0; phistep < 16; phistep++) {
		...
		for (thetastep = 0; thetastep < 32; thetastep++) {
			...
			sphere_vertices[vertexIndex++] = 2.0 * cos(phi)
					* cos(theta + deltaTheta);
			sphere_vertices[vertexIndex++] = 2.0 * cos(phi)
					* sin(theta + deltaTheta);
			sphere_vertices[vertexIndex++] = 2.0 * sin(phi);

			sphere_vertices[vertexIndex++] = 2.0 * cos(phi - deltaPhi)
					* cos(theta);
			sphere_vertices[vertexIndex++] = 2.0 * cos(phi - deltaPhi)
					* sin(theta);
			...
			sphere_normals[normalIndex++] = cos(phi - deltaPhi)
					* cos(theta + deltaTheta);
			sphere_normals[normalIndex++] = cos(phi - deltaPhi)
					* sin(theta + deltaTheta);
			sphere_normals[normalIndex++] = sin(phi - deltaPhi);
...
int GoodCitizen::initialize() {
	...
	if (returnCode == EXIT_SUCCESS) {
		...
		if (EXIT_SUCCESS
				!= loadTexture(
						"app/native/assets/images/goodcitizen/shadow.png", NULL,
						NULL, &tex_x, &tex_y, &shadow)) {
...
//	glDrawArrays(GL_TRIANGLE_STRIP, 0, 3);
	...
	if (!_model->compare("cube")) {
	...
	} else if (!_model->compare("pyramid")) {
		...
		for (phistep = 0; phistep < 16; phistep++) {
			for (thetastep = 0; thetastep < 32; thetastep++) {
				glDrawArrays(GL_TRIANGLE_STRIP, phistep * 128 + thetastep * 4,
						4);
			}
		}
	}
...
QVariantList& GoodCitizen::objectColor() {
	...
	*objectColor << obj_color[0] << obj_color[1] << obj_color[2]
			<< obj_color[3];
...
void GoodCitizen::update() {
	if (_autoRotate) {
		if (!_toolAxis->compare("X")) {
			...
			setStale(true);
		} else if (!_toolAxis->compare("Y")) {
			obj_angle_pitch = fmod((obj_angle_pitch + 1.0f), 360.0);

			setStale(true);
		} else if (!_toolAxis->compare("Z")) {
...
void GoodCitizen::onTouch(bb::cascades::TouchEvent *event) {
	...
	if (event->isDown()) {
		...
		qDebug() << "touch down: " << _touchMode->toLatin1() << " : "
	...
	if (event->isUp()) {
		...
		qDebug() << "touch up: " << _touchMode->toLatin1() << " : "
				<< event->screenX() << " : " << event->screenY();
	}
	if (event->isMove()) {
		...
		qDebug() << "move: " << _touchMode->toLatin1() << " : " << moveX
				<< " : " << moveY;
		...
		if (!_touchMode->compare("scale")) {
		...
		} else if (!_touchMode->compare("translate")) {
			if (lastTouchX != -1) {
				float deltaX = moveX / (100.0f);
				float deltaY = moveY / (100.0f);
				...
				shadow_pos_x = 70.0f
						+ (22.5f * 100.0f / (10.0f - obj_pos_z)) * (obj_pos_x)
						- 2.25f * (obj_pos_z);
				shadow_pos_y = 10.0f - 5.25f * (obj_pos_z);
				shadow_size_x = (float) 512 + 3.75f * obj_pos_z
						+ 2.25f * (obj_pos_y);
				shadow_size_y = (float) 256 + 2.0f * obj_pos_z
						+ 2.25f * (obj_pos_y);
		...
		} else if (!_touchMode->compare("rotate")) {
			...
			if (lastTouchX != -1) {
				...
				if (_toolAxis->compare("X")) {
					...
					obj_angle_pitch = fmod((obj_angle_pitch + deltaAngle),
							360.0);
